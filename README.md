...
- Первые две эпохи обучается только классификатор, затем размораживается backbone; early stopping останавливает обучение при отсутствии улучшения.
...
Результаты появятся в `runs/baseline`: `best.pt`, таблица истории, разбиения, отчёт по каждому классу и confusion matrix. Также автоматически создаются:
